...
func PgTableToStruct(t *PgTable, typeCfg *PgTypeMapConfig, keyConfig *AutoKeyMap, deprecated []string, queryer string, contextOnly bool) (*Struct, error) {
...
func PgCreateStruct(
	db Queryer, schema, typeMapPath, pkgName, customTmpl string, exTbls []string, autoGenKeyList []string, deprecated []string, queryer string, contextOnly bool) ([]byte, error) {
	...
	for _, tbl := range tbls {
		...
		st, err := PgTableToStruct(tbl, cfg, agkCfg, deprecated, queryer, contextOnly)
		if err != nil {
			return src, errors.WithStack(err)
		}
		if customTmpl != "" {
