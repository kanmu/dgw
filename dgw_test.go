...
func TestPgCreateStruct(t *testing.T) {
	...
	schema := "public"
	src, err := PgCreateStruct(conn, schema, "", "mypkg", "", []string{}, []string{}, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPgCreateStructWithAutoGenKey(t *testing.T) {
	...
	schema := "public"
	src, err := PgCreateStruct(conn, schema, "", "mypkg", "", []string{}, []string{"smallserial", "serial", "bigserial", "autogenuuid", "integer"}, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPgCreateStructNoIdentityAsPk(t *testing.T) {
	conn, cleanup := testPgSetup(t)
	defer cleanup()
	assert := assert.New(t)

	schema := "public"
	src, err := PgCreateStruct(conn, schema, "", "mypkg", "", []string{}, []string{}, false)
	if err != nil {
		t.Fatal(err)
	}
...
func (r *T5) CreateContext(ctx context.Context, db Queryer) error {
	_, err := db.ExecContext(ctx,
		` + "`INSERT INTO t5 (id, i) VALUES ($1, $2)`" + `,
		&r.ID, &r.I)
	if err != nil {
		return errors.WithStack(err)
	}
...
func (r *T5) CreateOnConflictDoNothing(ctx context.Context, db Queryer) (bool, error) {
	result, err := db.ExecContext(ctx,
		` + "`INSERT INTO t5 (id, i) VALUES ($1, $2) ON CONFLICT DO NOTHING`" + `,
		&r.ID, &r.I)
	if err != nil {
		return false, errors.WithStack(err)
	}
...
func (r *T6) CreateContext(ctx context.Context, db Queryer) error {
	_, err := db.ExecContext(ctx,
		` + "`INSERT INTO t6 (id, i) VALUES ($1, $2)`" + `,
		&r.ID, &r.I)
	if err != nil {
		return errors.WithStack(err)
	}
...
func (r *T6) CreateOnConflictDoNothing(ctx context.Context, db Queryer) (bool, error) {
	result, err := db.ExecContext(ctx,
		` + "`INSERT INTO t6 (id, i) VALUES ($1, $2) ON CONFLICT DO NOTHING`" + `,
		&r.ID, &r.I)
	if err != nil {
		return false, errors.WithStack(err)
	}
