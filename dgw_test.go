...
func TestPgCreateStruct(t *testing.T) {
	...
	schema := "public"
	src, err := PgCreateStruct(conn, schema, "", "mypkg", "", []string{}, []string{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPgCreateStructWithAutoGenKey(t *testing.T) {
	...
	schema := "public"
	src, err := PgCreateStruct(conn, schema, "", "mypkg", "", []string{}, []string{"smallserial", "serial", "bigserial", "autogenuuid", "integer"})
	if err != nil {
		t.Fatal(err)
	}
